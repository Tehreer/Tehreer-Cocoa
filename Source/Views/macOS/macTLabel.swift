...
private class SizeLabel: NSTextField {
...
open class TLabel: NSView {
    ...
    private func addSizeLabel() {
        ...
        _sizeLabel.alphaValue = 0.0
        _sizeLabel.isEnabled = false
        _sizeLabel.translatesAutoresizingMaskIntoConstraints = false
        _sizeLabel.maximumNumberOfLines = 0
    ...
    func intrinsicContentSize(for width: CGFloat) -> NSSize {
        ...
        return NSSize(width: sceil(textFrame.width), height: sceil(textFrame.height))
    }
    ...
    open override func draw(_ rect: NSRect) {
        ...
        if let context = NSGraphicsContext.current?.cgContext {
            renderer.renderScale = layer?.contentsScale ?? 1.0
    ...
    private func makeTextFrame(paddingLeft: CGFloat, paddingTop: CGFloat, layoutWidth: CGFloat, layoutHeight: CGFloat) -> ComposedFrame? {
        if let typesetter = typesetter {
            ...
            resolver.frameBounds = NSRect(x: paddingLeft, y: paddingTop, width: layoutWidth, height: layoutHeight)
    ...
    open func indexOfCodeUnit(at position: NSPoint) -> Int? {
    ...
    open func indexOfCharacter(at position: NSPoint) -> String.Index? {
